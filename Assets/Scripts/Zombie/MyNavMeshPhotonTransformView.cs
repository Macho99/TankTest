...
namespace Photon.Pun
{
	...
	public class MyNavMeshPhotonTransformView : MonoBehaviourPun, IPunObservable
	{
		...
		{
			var tr = transform;
			positionRefreshDiff = zombie.PositionRefreshDiff;
			if (!this.photonView.IsMine)
			{
				if (m_UseLocal)

				{
					Vector3 networkPosition = Vector3.MoveTowards(tr.localPosition, this.m_NetworkPosition, this.m_Distance * Time.deltaTime * PhotonNetwork.SerializationRate);
					zombie.NetworkPosition = networkPosition;
					if((tr.localPosition - networkPosition).magnitude > positionRefreshDiff)
					{
						agent.enabled = false;
			...
			{
				zombie.NetworkPosition = tr.position;
			}
		}
